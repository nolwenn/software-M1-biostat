...
In epidemiology and data sciences in general, you often come to convert numerical variable into to groups, i.e. a categorical variable. Here is an example with age as a continous variable that you split into age groups.
...
*ifelse()* function of conditionnal testing that returns either yes or no (or specified values) depending on whether the element of test is TRUE or FALSE. As example, we create below 

```{r}
sleep$overweight <- ifelse(sleep$bmi > 25, "Yes", "No")
sleep$overweight <- as.factor(sleep$overweight)
# For verification
