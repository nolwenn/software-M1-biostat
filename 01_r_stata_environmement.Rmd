...
**For small and large datasets**, the *str()* is useful to see the variables and their types (str for structure)
```{r, eval=F}
str(sleep)
```
...
:::{}
### Stata{-}
The *describe* function returns the main characteristics of the data set, *i.e.* the number of observations, number of variables, and characteristics of variables.
...
:::{}
### R{-}
In **R** if the arguments are written in the order expected by the developer of the function (default order) no need to specify the name of the argument. If you want to change one optional argument listed at the end of the function you need to specify its name or you will generate an error.
